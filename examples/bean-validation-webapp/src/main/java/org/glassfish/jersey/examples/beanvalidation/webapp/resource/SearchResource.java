...
public class SearchResource {
    ...
    @NotNull
    @NotEmptySearchField
    public List<ContactCard> searchForContact(
            @NotBlank(message = "{search.string.empty}") @QueryParam("q") final String searchValue) {

        if ("email".equals(searchType)) {
            return StorageService.findByEmail(searchValue);
        } else if ("phone".equals(searchType)) {
            return StorageService.findByPhone(searchValue);
        } else {
            return StorageService.findByName(searchValue);
        }
    }
}
