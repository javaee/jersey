...
/**
 * Resource representing permanent page to a shortened link. Client is shown {@code error-404} page if the provided link is
 * not valid (see {@link ErrorTemplate} and {@link ValidShortenedLink}).
 * <p/>
...
public class PermanentLinkResource {
    ...
    public URI getOriginal() {
        return getLink().getOriginal();
    }

    public URI getShortened() {
        return getLink().getShortened();
    }

    public URI getPermanent() {
        return getLink().getPermanent();
    }

    private ShortenedLink getLink() {
        return ShortenerService.getLink(link);
    }
}
