...
import org.glassfish.jersey.media.linking.internal.el.LinkBuilder;
...
public class LinkProcessor<T> {
    ...
    static String getLinkHeaderValue(LinkDescriptor desc, Object entity, Object resource, UriInfo uriInfo) {
        ...
        if (header.rev().length() != 0)
            builder = builder.param("rev", header.rev());
        if (header.type().length() != 0)
            builder = builder.type(header.type());
        if (header.title().length() != 0)
            builder = builder.param("title", header.title());
        if (header.anchor().length() != 0)
            builder = builder.param("anchor", header.anchor());
        if (header.media().length() != 0)
            builder = builder.param("media", header.media());
        if (header.hreflang().length() != 0)
            builder = builder.param("hreflang", header.hreflang());
