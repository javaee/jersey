...
final class JerseyRxCompletableFutureInvoker extends AbstractRxInvoker<CompletableFuture> implements RxCompletableFutureInvoker {
    ...
    JerseyRxCompletableFutureInvoker(final Invocation.Builder builder, final ExecutorService executor) {
    ...
    public <T> CompletableFuture<T> method(final String name, final Entity<?> entity, final Class<T> responseType) {
        return getExecutorService() == null
                ? CompletableFuture.supplyAsync(new CompletableFuture.Generator<T>() {
                        @Override
                        public T get() {
