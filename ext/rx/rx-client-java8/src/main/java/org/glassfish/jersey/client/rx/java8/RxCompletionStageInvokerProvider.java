...
/**
 * Invoker provider for invokers based on Java 8's {@code CompletionStage}.
 *
...
public final class RxCompletionStageInvokerProvider implements RxInvokerProvider {
    ...
    public <T> T getInvoker(final Class<T> invokerType, final Invocation.Builder builder, final ExecutorService executor) {
        if (RxCompletionStageInvoker.class.isAssignableFrom(invokerType)) {
            return invokerType.cast(new JerseyRxCompletionStageInvoker(builder, executor));
        }
        return null;
    }
}
