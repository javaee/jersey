...
public final class WebResourceFactory implements InvocationHandler {
    ...
    public Object invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
        ...
        for (int i = 0; i < paramAnns.length; i++) {
            ...
            if (!hasAnyParamAnnotation(anns)) {
                ...
                newTarget = configureRequestObjectsFromParamAnnos(value, anns, newTarget, form, cookies, headers);
            }
        }
    ...
    private WebTarget configureRequestObjectsFromParamAnnos(Object value,
            final Map<Class, Annotation> anns,
            WebTarget newTarget,
            final Form form,
            final LinkedList<Cookie> cookies,
            final MultivaluedHashMap<String, Object> headers) throws IllegalAccessException {
        ...
        if (value != null) {
            if ((ann = anns.get(PathParam.class)) != null) {
                ...
                for (Field field : value.getClass().getDeclaredFields()) {
                    final Map<Class, Annotation> beanFieldAnnos = new HashMap<>();

                    for (final Annotation beanFieldAnno : field.getDeclaredAnnotations()) {
                        beanFieldAnnos.put(beanFieldAnno.annotationType(), beanFieldAnno);
                    }

                    newTarget = configureRequestObjectsFromParamAnnos(
                            field.get(value), beanFieldAnnos, newTarget, form, cookies, headers);
                }
            }
        }
